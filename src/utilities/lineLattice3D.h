 ...
 * of a line3D given by Line3D<T> and the mother cuboid of CuboidGeometry2D<T>.
 **/
template <typename T>
class LineLattice3D {
private:
  CuboidGeometry3D<T>&  _geometry;
...
public:
  ...
  /**
   * i.e. the grid width is set to CuboidGeometry2D<T>::getDeltaR.
   **/
  LineLattice3D(CuboidGeometry3D<T>& geometry,
                      Line3D<T>      line3D);
