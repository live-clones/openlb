...
#include "optimization/serialization.h"
...
template <
  ...
  template<typename,SolverMode> typename SOLVER,
  typename OBJECTIVE,
  template<typename,typename> typename PRIMAL_DYNAMICS,
  unsigned FieldDim
>
...
public:
   ...
   : _objective(objective)
  {
    ...
    _cellVolume = util::pow(
      this->_objectiveDomain->getSuperGeometry().getCuboidGeometry().getDeltaR(),
      dim);
  }
  ...
  std::shared_ptr<SuperF<dim,T,T>> derivativeByPopulations() override {
    ...
    std::shared_ptr<SuperF<dim,T,T>> djdf = std::make_shared<SuperLatticeFfromCallableF<T,DESCRIPTOR>> (
      ...
      [this](T* output, auto cell, int iC, const int* coords){
        ...
        if (this->_objectiveDomain->operator()(iCglob,coords[0],coords[1],coords[2])) {
