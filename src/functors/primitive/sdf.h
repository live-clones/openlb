...
Vector<bool, 3> isSymmetric(std::function<T(const Vector<T, 3>&)> sdf,
                            Vector<T, 3> center) any_platform
{
...
*/
template <typename T>
T triangle(Vector<T, 2> p, Vector<T, 2> a, Vector<T, 2> b,
           Vector<T, 2> c) any_platform
{
  Vector<T, 2> e0  = b - a;
  Vector<T, 2> v0  = p - a;
  Vector<T, 2> e1  = c - b;
...
*/
template <typename T>
T cone(Vector<T, 3> p, Vector<T, 3> a, Vector<T, 3> ba, T baba, T ra,
       T rb) any_platform
{
  T rba  = rb - ra;
  T papa = (p - a) * (p - a);
...
T elongation(std::function<T(const Vector<T, 3>&)> sdf, const Vector<T, 3>& p,
             const Vector<T, 3>& h,
             const Vector<T, 3>& center = (T(0))) any_platform
{
...
T scale(std::function<T(const Vector<T, D>&)> sdf, const Vector<T, D>& p, T s,
        const Vector<T, D>& center = (T(0))) any_platform
{
