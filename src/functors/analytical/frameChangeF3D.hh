...
bool CircleCasson3D<T>::operator()(T output[], const T x[])
{
  ...
  T preFac = _pressureDrop / _cassonViscosity;
  for (std::size_t d = 0; d < 3; ++d) {
    if (r[d] < r_c) {
      r[d] = r_c;
    }
    output[d] = -1.0 * _scale * _normal[d] * preFac * ((_radius*_radius - r[d]*r[d]) / 2.0 +
      r_c * (_radius - r[d]) - (4.0 / 3.0) * util::sqrt(r_c) * (util::sqrt(util::pow(_radius, 3.0))
      - util::sqrt(util::pow(r[d], 3.0))));
  }
  return true;
}
