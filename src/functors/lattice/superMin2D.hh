...
#ifndef SUPER_MIN_2D_HH
#define SUPER_MIN_2D_HH
...
SuperMin2D<T,W>::SuperMin2D(FunctorPtr<SuperF2D<T,W>>&&        f,
    ...
    _indicatorF(std::move(indicatorF))
{
  ...
  auto& cuboid = _f->getSuperStructure().getCuboidGeometry();

  if ( _f->getBlockFSize()          == load.size() &&
       _indicatorF->getBlockFSize() == load.size() ) {
    for (int iC = 0; iC < load.size(); ++iC) {
      this->_blockF.emplace_back(
        new BlockMin2D<T,W>(_f->getBlockF(iC),
...
template <typename T, typename W>
bool SuperMin2D<T,W>::operator() (W output[], const int input[])
{
