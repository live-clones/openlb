...
concept Field = requires() {
  ...
  // Size is exposed
  { FIELD::template size<2,9>() } -> std::convertible_to<std::size_t>;
...
struct Field {
  ...
  static constexpr auto getInitialValue() {
    return Vector<value_type<T>,size()>{};
  }
};
...
struct Parameters  {
  ...
  template <typename...>
  using include_fields = Parameters;
  ...
  template <typename FIELD>
  void set(FieldD<ValueType,Descriptor,FIELD>&&) { }
};
