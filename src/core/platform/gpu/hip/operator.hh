...
#ifndef GPU_HIP_OPERATOR_HH
#define GPU_HIP_OPERATOR_HH
 ...
 *
 * \code{.cpp}
 * superLattice.forBlocksOnPlatform<Platform::GPU_HIP>([](auto& block) {
 *   block.setCollisionO(
 *     gpu::hip::getFusedCollisionO<T,DESCRIPTOR,
 ...
 **/
template <typename T, typename DESCRIPTOR, typename... DYNAMICS>
std::function<void(ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>&)>
getFusedCollisionO() {
  return [](ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block) {
...
template <typename CONTEXT, typename... ARGS>
void async_call_operators(hipStream_t stream, CONTEXT& lattice, bool* subdomain, ARGS&&... args) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
  const auto block_count = (lattice.getNcells() + block_size - 1) / block_size;
...
template <typename CONTEXT, typename... ARGS>
void async_call_operators_with_statistics(hipStream_t stream, CONTEXT& lattice, bool* subdomain, ARGS&&... args) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
  const auto block_count = (lattice.getNcells() + block_size - 1) / block_size;
...
template <typename CONTEXT, typename... ARGS>
void call_list_operators(CONTEXT& lattice,
                         const gpu::hip::Column<CellID>& cells,
                         ARGS&&... args) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
...
template <typename CONTEXT, typename... ARGS>
void async_call_list_operators(hipStream_t stream,
                               CONTEXT& lattice,
                               const gpu::hip::Column<CellID>& cells,
                               ARGS&&... args) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
...
template <typename CONTEXT, typename... ARGS>
void async_call_list_operators_with_statistics(hipStream_t stream,
                                               CONTEXT& lattice,
                                               const gpu::hip::Column<CellID>& cells,
                                               ARGS&&... args) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
...
template <typename T, typename DESCRIPTOR, typename DYNAMICS>
ConcreteBlockCollisionO<T,DESCRIPTOR,Platform::GPU_HIP,DYNAMICS>::ConcreteBlockCollisionO():
...
template <typename T, typename DESCRIPTOR, typename DYNAMICS>
void ConcreteBlockCollisionO<T,DESCRIPTOR,Platform::GPU_HIP,DYNAMICS>::applyDominant(
  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block,
  ConcreteBlockMask<T,Platform::GPU_HIP>&               subdomain)
{
  using namespace gpu::hip;
  DeviceContext<T,DESCRIPTOR> lattice(block);
  if (block.statisticsEnabled()) {
    call_operators_with_statistics(
      lattice,
...
template <typename T, typename DESCRIPTOR, typename DYNAMICS>
void ConcreteBlockCollisionO<T,DESCRIPTOR,Platform::GPU_HIP,DYNAMICS>::applyIndividual(
  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block,
  ConcreteBlockMask<T,Platform::GPU_HIP>&               subdomain)
{
  using namespace gpu::hip;
...
template <typename T, typename DESCRIPTOR, typename DYNAMICS>
void ConcreteBlockCollisionO<T,DESCRIPTOR,Platform::GPU_HIP,DYNAMICS>::setup(
  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block)
{
...

  {
    ...
    _deviceDynamics = gpu::hip::device::malloc<gpu::hip::ConcreteDynamics<T,DESCRIPTOR,DYNAMICS>>(1);
    gpu::hip::kernel::construct_dynamics<T,DESCRIPTOR,DYNAMICS><<<1,1>>>(
      _deviceDynamics.get(),
      _parameters->deviceData());
    gpu::hip::device::check();

    // Fetch pointer to on-device dynamic-dispatch field
    _dynamicsOfCells = block.template getField<gpu::hip::DYNAMICS<T,DESCRIPTOR>>()[0].data();
  }
}

template <typename T, typename DESCRIPTOR, concepts::CellOperator OPERATOR>
ConcreteBlockO<T,DESCRIPTOR,Platform::GPU_HIP,OPERATOR,OperatorScope::PerCell>::ConcreteBlockO():
  _cells(0),
  _modified{false},
  _stream{hipStreamDefault}
{ }
...
template <typename T, typename DESCRIPTOR, concepts::CellOperator OPERATOR>
void ConcreteBlockO<T,DESCRIPTOR,Platform::GPU_HIP,OPERATOR,OperatorScope::PerCell>::apply(
  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block)
{
  if (_cells.size() > 0) {
    if (_modified) {
    ...
    gpu::hip::DeviceBlockLattice<T,DESCRIPTOR> lattice(block);
    gpu::hip::async_call_list_operators(_stream.get(),
                                         lattice,
                                         _cells,
                                         gpu::hip::ListedPostProcessor<OPERATOR>{});
  }
}

template <typename T, typename DESCRIPTOR, concepts::CellOperator OPERATOR>
ConcreteBlockO<T,DESCRIPTOR,Platform::GPU_HIP,OPERATOR,OperatorScope::PerCellWithParameters>::ConcreteBlockO():
  _cells(0),
  _modified{false},
  _stream{hipStreamDefault}
{ }
...
template <typename T, typename DESCRIPTOR, concepts::CellOperator OPERATOR>
void ConcreteBlockO<T,DESCRIPTOR,Platform::GPU_HIP,OPERATOR,OperatorScope::PerCellWithParameters>::apply(
  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block)
{
  if (_cells.size() > 0) {
    if (_modified) {
...
ConcreteBlockCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCell>
...
template <typename COUPLER, typename COUPLEES>
void ConcreteBlockCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCell>::set(
...
template <typename COUPLER, typename COUPLEES>
void ConcreteBlockCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCell>::apply()
{
  auto deviceLattice = _lattices.exchange_values([&](auto name) -> auto {
    return gpu::hip::DeviceBlockLattice{*_lattices.get(name)};
  });
  if (_mask) {
    _mask->setProcessingContext(ProcessingContext::Simulation);
    gpu::hip::call_coupling_operators(
      deviceLattice, _mask->deviceData(),
      gpu::hip::UnmaskedCoupling<COUPLER>{});
  } else {
...
template <typename COUPLER, typename COUPLEES>
template <typename LATTICES>
ConcreteBlockCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCellWithParameters>
::ConcreteBlockCouplingO(LATTICES&& lattices)
  : _lattices{lattices}
{
  // If the lattices are introspectable, allocate fields accessed by the COUPLER
...
template <typename COUPLER, typename COUPLEES>
void ConcreteBlockCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCellWithParameters>::set(
  CellID iCell, bool state)
{
  if (!_mask) {
    _mask = std::make_unique<ConcreteBlockMask<typename COUPLEES::values_t::template get<0>::value_t,
...
template <typename COUPLER, typename COUPLEES>
void ConcreteBlockCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCellWithParameters>::apply()
{
  auto deviceLattice = _lattices.exchange_values([&](auto name) -> auto {
    return gpu::hip::DeviceBlockLattice{*_lattices.get(name)};
  });
  if (_mask) {
    ...
    gpu::hip::call_coupling_operators(
      deviceLattice, _mask->deviceData(),
      gpu::hip::UnmaskedCouplingWithParameters<COUPLER,COUPLEES>{_parameters});
  } else {
    auto& mask = _lattices.template get<0>()->template getData<CollisionSubdomainMask>();
    gpu::hip::call_coupling_operators(
      deviceLattice, mask.deviceData(),
...
template <typename COUPLER, typename COUPLEES>
void ConcreteBlockPointCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCellWithParameters>::set(
  CellID iCell, bool state)
{
...
template <typename COUPLER, typename COUPLEES>
void ConcreteBlockPointCouplingO<COUPLEES,Platform::GPU_HIP,COUPLER,OperatorScope::PerCellWithParameters>::apply()
{
  ...
  auto deviceLattice = _lattices.exchange_values([&](auto name) -> auto {
    if constexpr (std::is_same_v<typename decltype(name)::type, names::Points>) {
      return gpu::hip::DeviceContext{*_lattices.get(name)};
    } else {
      return gpu::hip::DeviceBlockLattice{*_lattices.get(name)};
    }
  });

  gpu::hip::call_particle_coupling_operators(
    deviceLattice,
    gpu::hip::ParticleCouplingWithParameters<COUPLER,COUPLEES>{_parameters});
}
