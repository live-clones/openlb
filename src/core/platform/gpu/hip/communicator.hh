...
#ifndef PLATFORM_GPU_HIP_COMMUNICATOR_HH
#define PLATFORM_GPU_HIP_COMMUNICATOR_HH
...
template <>
void checkPlatform<Platform::GPU_HIP>()
{
  OstreamManager clout(std::cout, "GPU_HIP");
  ...
  if (nDevices < 1) {
    clout << "No HIP device found" << std::endl;
  } else if (nDevices > 1) {
    clout << "Found " << nDevices << " HIP devices but only one can be used per MPI process." << std::endl;
  }
#ifdef OLB_DEBUG
  for (int deviceI=0; deviceI < nDevices; ++deviceI) {
    hipDeviceProp_t deviceProp;
    hipGetDeviceProperties(&deviceProp, deviceI);
...
template <typename FIELD, typename CONTEXT>
void async_gather_field(hipStream_t stream,
                        CONTEXT& lattice,
...
void async_copy_field(hipStream_t stream,
...
void async_gather_any_fields(hipStream_t stream,
...
void async_copy_any_fields(hipStream_t stream,
...
template <typename FIELD, typename CONTEXT>
void async_scatter_field(hipStream_t stream,
                         CONTEXT& lattice,
                         const thrust::device_vector<CellID>& indices,
                         std::uint8_t* buffer) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
...
void async_scatter_any_fields(hipStream_t stream,
                              thrust::device_vector<AnyDeviceFieldArrayD>& fields,
                              const thrust::device_vector<CellID>& indices,
                              std::uint8_t* buffer) {
  const auto block_size = DEFAULT_HIP_WARP_SIZE;
...
class ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::SendTask {
private:
  thrust::device_vector<gpu::hip::AnyDeviceFieldArrayD> _fields;
  const bool _onlyPopulationField;

  const thrust::device_vector<CellID> _cells;

  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& _source;

  gpu::hip::device::unique_ptr<std::uint8_t> _buffer;
  std::unique_ptr<MpiSendRequest> _request;

  std::unique_ptr<gpu::hip::device::Stream> _stream;

public:
  SendTask(MPI_Comm comm, int tag, int rank,
           const std::vector<std::type_index>& fields,
           const std::vector<CellID>& cells,
           ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& block):
    ...
    _stream(std::make_unique<gpu::hip::device::Stream>(hipStreamNonBlocking))
  {
    ...
    _buffer = gpu::hip::device::malloc<std::uint8_t>(size);
  ...
  void prepare()
  {
    if (_onlyPopulationField) {
      gpu::hip::DeviceContext<T,DESCRIPTOR> lattice(_source);
      gpu::hip::async_gather_field<descriptors::POPULATION>(_stream->get(), lattice, _cells, _buffer.get());
    } else {
      gpu::hip::async_gather_any_fields(_stream->get(), _fields, _cells, _buffer.get());
    }
  }
...
template <typename T, typename DESCRIPTOR>
class ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::RecvTask {
private:
  const int _tag;
  const int _rank;
  ...
  ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& _target;
...
public:
  ...
  {
    if (_onlyPopulationField) {
      gpu::hip::DeviceContext<T,DESCRIPTOR> lattice(_target);
      gpu::hip::async_scatter_field<descriptors::POPULATION>(_stream->get(), lattice, _cells, _buffer.get());
    } else {
      gpu::hip::async_scatter_any_fields(_stream->get(), _fields, _cells, _buffer.get());
    }
  }
...
template <typename T, typename DESCRIPTOR>
struct ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::CopyTask {
  virtual ~CopyTask() { }
...
/// Wrapper for a local plain-copy block communication request
template <typename T, typename DESCRIPTOR>
class ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::HomogeneousCopyTask
  : public ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::CopyTask {
private:
  thrust::device_vector<gpu::hip::AnyDeviceFieldArrayD> _sourceFields;
  thrust::device_vector<gpu::hip::AnyDeviceFieldArrayD> _targetFields;
...
public:
  HomogeneousCopyTask(
    const std::vector<std::type_index>& fields,
    const std::vector<CellID>& targetCells, ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& target,
    const std::vector<CellID>& sourceCells, ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& source):
  ...
  {
    if (_onlyPopulationField) {
      gpu::hip::DeviceContext<T,DESCRIPTOR> sourceLattice(_source);
      gpu::hip::DeviceContext<T,DESCRIPTOR> targetLattice(_target);
      gpu::hip::async_copy_field<descriptors::POPULATION>(_stream->get(), sourceLattice, targetLattice, _sourceCells, _targetCells);
    } else {
      gpu::hip::async_copy_any_fields(_stream->get(), _sourceFields, _targetFields, _sourceCells, _targetCells);
    }
  }
...
private:
  ...
  gpu::hip::Column<std::uint8_t> _buffer;
...
public:
  ...
  void copy() override {
    ...
    if (_onlyPopulationField) {
      gpu::hip::DeviceContext<T,DESCRIPTOR> lattice(_target);
      gpu::hip::async_scatter_field<descriptors::POPULATION>(_stream->get(), lattice, _targetCells, _buffer.deviceData());
    } else {
      gpu::hip::async_scatter_any_fields(_stream->get(), _targetFields, _targetCells, _buffer.deviceData());
    }
  }
...
public:
  ...
  void copy() override {
    if (_onlyPopulationField) {
      gpu::hip::DeviceContext<T,DESCRIPTOR> lattice(_source);
      gpu::hip::async_gather_field<descriptors::POPULATION>(_stream->get(), lattice, _sourceCells, _buffer.deviceData());
    } else {
      gpu::hip::async_gather_any_fields(_stream->get(), _sourceFields, _sourceCells, _buffer.deviceData());
    }
...
template <typename T, typename DESCRIPTOR, Platform SOURCE>
HeterogeneousCopyTask<T,DESCRIPTOR,SOURCE,Platform::GPU_HIP>::HeterogeneousCopyTask(
  const std::vector<std::type_index>& fields,
  const std::vector<CellID>& targetCells, ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& target,
  const std::vector<CellID>& sourceCells, ConcreteBlockLattice<T,DESCRIPTOR,SOURCE>&             source):
...
template <typename T, typename DESCRIPTOR, Platform SOURCE>
void HeterogeneousCopyTask<T,DESCRIPTOR,SOURCE,Platform::GPU_HIP>::copy()
{
  _impl->copy();
}

template <typename T, typename DESCRIPTOR, Platform SOURCE>
void HeterogeneousCopyTask<T,DESCRIPTOR,SOURCE,Platform::GPU_HIP>::wait()
{
  _impl->wait();
}

template <typename T, typename DESCRIPTOR, Platform TARGET>
HeterogeneousCopyTask<T,DESCRIPTOR,Platform::GPU_HIP,TARGET>::HeterogeneousCopyTask(
  const std::vector<std::type_index>& fields,
  const std::vector<CellID>& targetCells, ConcreteBlockLattice<T,DESCRIPTOR,TARGET>&             target,
  const std::vector<CellID>& sourceCells, ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>& source):
...
template <typename T, typename DESCRIPTOR, Platform TARGET>
void HeterogeneousCopyTask<T,DESCRIPTOR,Platform::GPU_HIP,TARGET>::copy()
{
  _impl->copy();
}

template <typename T, typename DESCRIPTOR, Platform TARGET>
void HeterogeneousCopyTask<T,DESCRIPTOR,Platform::GPU_HIP,TARGET>::wait()
{
  _impl->wait();
}
...
template <typename T, typename DESCRIPTOR>
ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::ConcreteBlockCommunicator(
    ...
    if (loadBalancer.isLocal(remoteC)) {
      ...
      if (!neighborhood.getCellsOutboundTo(remoteC).empty()) {
        _sendTasks.emplace_back(std::make_unique<SendTask>(
          ...
          super.template getBlock<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>(_iC)));
      }
      if (!neighborhood.getCellsInboundFrom(remoteC).empty()) {
  ...
  neighborhood.forNeighbors([&](int localC) {
    if (!neighborhood.getCellsInboundFrom(localC).empty()) {
      _copyTasks.emplace_back(new HomogeneousCopyTask(
        ...
        neighborhood.getCellsInboundFrom(localC),   super.template getBlock<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>(_iC),
        neighborhood.getCellsRequestedFrom(localC), super.template getBlock<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>(loadBalancer.loc(localC))));
    }
  });
#endif
}
...
template <typename T, typename DESCRIPTOR>
void ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::receive()
{
...
template <typename T, typename DESCRIPTOR>
void ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::send()
{
...
template <typename T, typename DESCRIPTOR>
void ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::unpack()
{
...
template <typename T, typename DESCRIPTOR>
void ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::wait()
{
  for (auto& task : _copyTasks) {
    task->wait();
  }
...
template <typename T, typename DESCRIPTOR>
void ConcreteBlockCommunicator<ConcreteBlockLattice<T,DESCRIPTOR,Platform::GPU_HIP>>::copy()
{
  for (auto& task : _copyTasks) {
    task->copy();
  }
