...
private:
  ...
  std::function<void(Cell<Expr,DESCRIPTOR>&)> _setPlaceholderExpr;
...
public:
  ...
  FieldTypePromise(meta::id<FIELD_TYPE>):
    ...
    _setPlaceholderExpr([](Cell<Expr,DESCRIPTOR>& cell) {
      if constexpr (concepts::LatticeDescriptor<DESCRIPTOR>) {
        if constexpr (   std::is_same_v<Expr, T>
                      && std::is_base_of_v<field_type_of_field_array_tag, FIELD_TYPE>) {
          if constexpr (std::is_same_v<Expr, typename FieldD<Expr,DESCRIPTOR,typename FIELD_TYPE::field_t>::value_t>) {
            ...
            for (unsigned iD=0; iD < placeholder.d; ++iD) {
              placeholder[iD] = Expr(  "Symbol(\"cell.template getFieldComponent<"
                                     + fields::name<typename FIELD_TYPE::field_t>()
                                     + ">(" + std::to_string(iD) +  ")\")");
            }
  ...
  void setPlaceholderExpression(Cell<Expr,DESCRIPTOR>& cell) {
    _setPlaceholderExpr(cell);
  }
