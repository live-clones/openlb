...
void vtkSurfaceWriter<T>::init()
{
  ...
  for (int globC=0; globC < _cuboidGeometry.cuboids().size(); ++globC) {
    if (_loadBalancer.isLocal(globC)) {
      localCuboids.emplace_back(&_cuboidGeometry.cuboids()[globC]);
    }
  }
  ...
  for (STLtriangle<T>& triangle : _surfaceI.getMesh().getTriangles()) {
    ...
    for (int iD=0; iD < 3; ++iD) {
      ...
      for (auto* cuboid : localCuboids) {
        if (cuboid->checkPoint(triangle.point[iD].coords, 1)) {
    ...
    if (isTriangleContained) {
      ...
      for (const auto& point : triangle.point) {
        _points->InsertNextPoint(point.coords[0], point.coords[1], point.coords[2]);
      }
