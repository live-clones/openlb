...
#ifndef TURBULENT_WALL_MODEL_POST_PROCESSOR_3D_H
#define TURBULENT_WALL_MODEL_POST_PROCESSOR_3D_H
...
namespace olb {
  ...
  void interpolateVelocity(CELL& cell, Vector<V,3> distance, V u_y2[3]) any_platform{
    Vector<V,3> floorV(util::floor(distance[0]), util::floor(distance[1]), util::floor(distance[2]));
    ...
    for (auto point : surroundingPoints) {
      ...
      const V weight = (V(1.) - util::abs(dist[0])) * (V(1.) - util::abs(dist[1])) * (V(1.) - util::abs(dist[2]));
      V uNP[3] = {0.};
...
//======================================================================
  ...
  template<bool bodyForce, bool interpolateSampleVelocity, bool useVanDriest, int wallFunctionProfile>
  ...
  public:
    ...
    void apply(CELL& cell, PARAMETERS& parameters) any_platform{
      ...
      Vector<V,3> y2 = samplingCellDistance*normal;
      V u_plusF {};
      if( y1Norm != V(0.) ){
        auto y1y2 = y2 - y1;
        V u_y2[3] = {0.};
        if( interpolateSampleVelocity ) {
          ...
          cell.neighbor({int(util::round(y1y2[0])), int(util::round(y1y2[1])), int(util::round(y1y2[2]))}).computeU(u_y2);
        }
        V u_y2Norm =  util::norm<DESCRIPTOR::d>(u_y2);
        if( u_y2Norm >= V(1.e-7)) {
          V u_1Actual[3] = {0.};
          cell.computeU(u_1Actual);
          auto u_t = u_y2 - (u_y2[0]*normal[0] + u_y2[1]*normal[1] + u_y2[2]*normal[2])*normal;
          ...
          V u_1t = u_1Actual[0]*tangent[0] + u_1Actual[1]*tangent[1] + u_1Actual[2]*tangent[2];
          if ( u_2 != 0. ) {
          ...
          if(util::norm<DESCRIPTOR::d>(u_t) != 0. && u_1 > 0.){
            ...
            if( bodyForce ) {
              ...
              V u_b[3] = {ub[0]-V(0.5)*force[0]*tangent[0], ub[1]-V(0.5)*force[1]*tangent[1], ub[2]-V(0.5)*force[2]*tangent[2]};
              cell.template setField<descriptors::VELOCITY>(u_b);
            } else {
              V u_b[3] = {ub[0], ub[1], ub[2]};
              cell.template setField<descriptors::VELOCITY>(u_b);
            }
          }
        }
...
//======================================================================
  ...
  public:
    ...
    void apply(CELL& cell, PARAMETERS& parameters) any_platform{
      ...
      if ( uNorm < V(1.e-7) ) {
        cell.template setField<descriptors::POROSITY>(V(1));
        ...
        cell.template setField<descriptors::POROSITY>(V(0));
        ...
        if( y1Norm != V(0.)){
          ...
          if( rhoMethod == 0) {
            ...
            rho = cell.neighbor({int(util::round(normal[0])), int(util::round(normal[1])), int(util::round(normal[2]))}).computeRho();
          ...
          cell.neighbor({int(util::round(normal[0])), int(util::round(normal[1])), int(util::round(normal[2]))}).computeStress(pi);
          pi[xx] /= factor;
          pi[yy] /= factor;
          pi[zz] /= factor;
          pi[xy] /= factor;
          ...
          cell.template setField<descriptors::STRAINRATE>(pi);
        }
      }
    }
  };
    ...
    void apply(CELL& cell) any_platform{
      ...
      if ( uNorm < V(1.e-7) ) {
        ...
        if( y1Norm != V(0.)){
          ...
          if( rhoMethod == 1) {
            Vector<V,3> normal(y1[0]/y1Norm, y1[1]/y1Norm, y1[2]/y1Norm);
            V extRho = cell.neighbor({int(util::round(normal[0])), int(util::round(normal[1])), int(util::round(normal[2]))}).computeRho();
...
//======================================================================
  ...
  class TurbulentSimpleWallModelPostProcessor {
  public:
    static constexpr OperatorScope scope = OperatorScope::PerCellWithParameters;
    using parameters = meta::list<descriptors::OMEGA, descriptors::SAMPLING_DISTANCE>;
    ...
    void apply(CELL& cell, PARAMETERS& parameters) any_platform{
      ...
      if( y1Norm != V(0.) ){
        ...
        V pi[util::TensorVal<DESCRIPTOR>::n] {V(0)};
        V rho {};
        cell.neighbor({int(util::round(y1y2[0])), int(util::round(y1y2[1])), int(util::round(y1y2[2]))}).computeAllMomenta(rho, u_y2, pi);
        V u_y2Norm =  util::norm<DESCRIPTOR::d>(u_y2);
        if( u_y2Norm >= V(1.e-7)) {
          V u_1Actual[3] = {0.};
          cell.computeU(u_1Actual);
