...
public:
  ...
  // interpolate velocity value at point given by distance from neighbor nodes
  template <typename CELL, typename V = typename CELL::value_t>
  static void interpolateVelocityOnPoint(CELL& cell, Vector<V, 3> distance,
                                         Vector<V, 3>& u) any_platform
  {
    ...
    for (int nP = 0; nP < 8; nP++) {
      ...
      V volume = (V(1) - util::abs(dist[0])) * (V(1) - util::abs(dist[1])) *
                 (V(1) - util::abs(dist[2]));
      Vector<V, 3> uNP(0., 0., 0.);
      cell.neighbor(surroundingPoints[nP]).computeU(uNP.data());
      //V rho = cell.neighbor(surroundingPoints[nP]).computeRho();
      u += uNP * volume;
    }
  };
    ...
    for (int iPop = 1; iPop < DESCRIPTOR::q; ++iPop) {
      ...
      if ((opp_bouzidi_dist >= 0) && (normal_norm != 0)) {
        ...
        if (norm(distance) != V {0.}) {
          interpolateVelocityOnPoint(cell, distance, u1_latt);
          u1_phys  = u1_latt * conversionFactorVelocity;
          distance = 2 * delta * normal + distance_bc;
          //distance = distance_bc + normal * charLength/(4*conversionFactorLength);
          interpolateVelocityOnPoint(cell, distance, u2_latt);
