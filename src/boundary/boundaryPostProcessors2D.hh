                                   ...
                                   orientation>::apply(CELL& cell)
{
  ...
  T dy_uy = dy_u[1];
  T omega =
      dynamics.getOmegaOrFallback(std::numeric_limits<T>::signaling_NaN());
    ...
    CELL& cell)
{
  ...
  T dx_u[DESCRIPTOR::d], dy_u[DESCRIPTOR::d];
  fd::DirectedGradients2D<T, DESCRIPTOR, 0, xNormal, true>::interpolateVector(
      dx_u, cell);
  fd::DirectedGradients2D<T, DESCRIPTOR, 1, yNormal, true>::interpolateVector(
      dy_u, cell);
  T dx_ux = dx_u[0];
  T dy_ux = dy_u[0];
  T dx_uy = dx_u[1];
