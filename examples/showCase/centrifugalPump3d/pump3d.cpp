...
int main(int argc, char* argv[])
{
  ...
  SuperPorousElementForceReductionO fsiReductionO(sLattice,
  ...
  if (fsiReductionO.rankDoesFSI()) {
    ...
    auto rotorDeviceD = rotorD->template getField<descriptors::POROSITY>()
                               .template asConcrete<Platform::GPU_CUDA>()[0].deviceData();
    #else
    auto rotorDeviceD = rotorD->template getField<descriptors::POROSITY>()
                               .template asConcrete<Platform::CPU_SISD>()[0].data();
    #endif
...

    {
      ...
      auto params = makeParametersD<T,descriptors::SPATIAL_DESCRIPTOR<3>>(
        ...
        fields::fsi::ELEMENT_REFERENCE_POROSITIES{}, rotorDeviceD
      );
  ...
  for (std::size_t iT = 0; iT <= converter.getLatticeTime(maxPhysT); ++iT) {
    ...
    if (fsiReductionO.rankDoesFSI()) {
      ...
      auto t = converter.getPhysTorque(fsiReductionO.getTorque(iElement));
