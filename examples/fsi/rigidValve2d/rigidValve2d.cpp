...
using DESCRIPTOR = descriptors::D2Q9<
  ...
  fields::fsi::BOUNDARY_FORCE,
  fields::fsi::BOUNDARY_TORQUE
>;
                ...
                bool vtkEnabled)
{
  ...
  if (iT % vtkIter == 0 && vtkEnabled) {
    ...
    }
    {
      ...
      SuperLatticeField2D<T,DESCRIPTOR,fields::fsi::BOUNDARY_FORCE> force(superLattice);
      SuperLatticeField2D<T,DESCRIPTOR,fields::fsi::BOUNDARY_TORQUE> torque(superLattice);
...
int main(int argc, char* argv[]) {
  ...
  CuboidGeometry2D<T> cuboidGeometry(*paddedDomainI, converter.getPhysDeltaX(), singleton::mpi().getSize());
  ...
  SuperPorousElementForceReductionO fsiReductionO(superLattice,
  ...
  for (std::size_t iT = 0; iT <= converter.getLatticeTime(maxPhysT); ++iT) {
    ...
    if (fsiReductionO.rankDoesFSI()) {
      for (unsigned iElement=1; iElement <= fsiReductionO.getElementCount(); ++iElement) {
        ...
        auto f = converter.getPhysForce(fsiReductionO.getForce(iElement)) / converter.getPhysDeltaX();
        auto t = converter.getPhysTorque(fsiReductionO.getTorque(iElement)) / converter.getPhysDeltaX();

        auto angularAccel = t[0] / valveMomentaOfInertia;
