 ...
 *  0=eternal (4x domain size to capture the vanishing shock)
 *  1=periodic,
 *  2=local,
 *  3=damping (with periodic boundaries),
 *  4=dampingAndLocal,
 *  5=dampingAndZeroGradient,
 */
 ...
 typedef enum {eternal, periodic, local, damping, dampingAndLocal, dampingAndZeroGrad} BoundaryType;
                      ...
                      )
 {
   ...
   if ( source == point || source == movingpoint ) {
     ...
     IndicatorSphere3D<T> pointSourceIndicator( originSource, dx2 );
     superGeometry.rename( checMat, sourMat, pointSourceIndicator );
   }
   
   switch ( boundarytype ) {
     case eternal:
       superGeometry.rename( checMat, dampMat, fluidDomain );  // just to be able to get uAverage for inside
     ...
     case local: {
       ...
       IndicatorCuboid3D<T> bottomflow( extend, origin );
       ...
       IndicatorCuboid3D<T> topflow( extend, origin );
       ...
       IndicatorCuboid3D<T> frontflow( extend, origin );
       ...
       IndicatorCuboid3D<T> backflow( extend, origin );
                   ...
                   )
 {
   ...
   if ( boundarytype == dampingAndLocal || boundarytype == eternal ) fluidMaterial = fluiMat;
                     ...
                     )
 {
   ...
   std::unique_ptr<SuperIndicatorF<T,ndim>> bulkIndicator = superGeometry.getMaterialIndicator({dampMat,fluiMat});
   sLattice.defineDynamics<BulkDynamics>( bulkIndicator );
 
   std::unique_ptr<SuperIndicatorF<T,ndim>> inletBoundaries = superGeometry.getMaterialIndicator({inflMat, outfMat, backMat, fronMat, topfMat, bottMat, checMat});
   ...
   switch ( boundarytype ) {
     ...
     case local:
       ...
       boundary::set<boundary::LocalVelocity>(sLattice, inletBoundaries );
                 ...
                 )
 {
   ...
   if ( ( boundarytype == dampingAndLocal || boundarytype == local ) && ( iT%50 == 0 ) ) {
     setUAverage( sLattice, superGeometry, boundarytype, fluidDomain );
 ...
 int main( int argc, char* argv[] )
 {
   ...
   const int res                 = args.getValueOrFallback( "--res",               100 );
   ...
   if ( outdir == "" ) {
     ...
     outdir_mod  << "_Ma" << Ma << "_Re" << Re << "_a" << amplitude
                 << "_" << lx << "x" << ly << "x" << lz
                 << "_res" << res;
     if ( boundaryCondition == 3 || boundaryCondition == 4 ) outdir_mod << "_boundaryDepth" << boundaryDepth << "x" << dampingStrength;
   } else {
     outdir_mod << outdir;
   }
   ...
   switch ( boundarytype ) {
     case eternal: case periodic: case damping:  cuboidGeometry.setPeriodicity({true, true, true});    break;
   ...
   CuboidGeometry<T,ndim> fluidGeometry( fluidDomain, converter.getConversionFactorLength(), noOfCuboids );
   SuperGeometry<T,ndim> superFluidGeometry( fluidGeometry, loadBalancer, overlap );
   SuperIndicatorF3D<T> fluidDomainIndicator( superFluidGeometry );
   ...
   prepareGeometry( converter, superGeometry, domain, fluidDomain, boundarytype, source, res, debug, boundaryDepth );
   ...
   prepareLattice( converter, sLattice, superGeometry, rho0, Ma, amplitude, alpha_shock, boundarytype, source, boundaryDepth, domainLengths, dampingStrength );
   ...
   T Lp0 = L2Norm( sLattice, superGeometry, 0, converter, superFluidGeometry );
