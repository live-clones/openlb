                     ...
                     )
{
  ...
  clout << "Prepare Geometry ..." << std::endl
        ...
        << "4 = Pressure Source Boundary" << std::endl;
  ...
  if ( source == pointsource ) {
    const T radiusPointSource = converter.getPhysDeltaX()*0.5;  // domain_lengths[0] / res;  // 
    Vector<T,3> originSource( 0., 0., 0.);
    std::shared_ptr<IndicatorF3D<T>> point_source = std::make_shared<IndicatorSphere3D<T>>( originSource, radiusPointSource );
    superGeometry.rename( 2, 1, fluid_domain-point_source );  // all nodes are fluid
    superGeometry.rename( 2, 4, point_source );  // 
  } else {
                    ...
                    )
{
  ...
  if ( boundarytype == bounceBack ) {
    ...
    clout << "Invalid boundarytype specified!" << std::endl;
  }
  ...
  if ( source == shock ) {
    ...
    //Initialize all values of distribution functions to their local equilibrium
    sLattice.defineRhoU( superGeometry.getMaterialIndicator({4}), rho, u );
    sLattice.iniEquilibrium( superGeometry.getMaterialIndicator({4}), rho, u );
  }

  // Define fields
  sLattice.setParameter<descriptors::OMEGA>( omega );
  AnalyticalConst3D<T,T> rhoField( rho0 );
  sLattice.defineField<descriptors::DENSITY>( superGeometry.getMaterialIndicator({1,2,3,4}), rhoField );
  clout << "Rho is set to " << rho0 << " on Materials 1,2,3,4" << std::endl;
  T actualV(0.);
  if ( source == shock ) {
    actualV = Ma;
  } else if ( source == pointsource ) {
  ...
  sLattice.defineField<descriptors::UX>( superGeometry.getMaterialIndicator({1,2,3,4}), uxField );
  clout << "UX is set to " << actualV << " on Materials 1,2,3,4" << std::endl;
                        ...
                        T frequency )
{
  ...
  if ( boundarytype != periodic and boundarytype != bounceBack and boundarytype != damping ) {
  ...
  if ( source == pointsource ) {
    ...
    AnalyticalConst3D<T,T> uy( 0. );
    AnalyticalConst3D<T,T> uz( 0. );
    AnalyticalComposed3D<T,T> u3d( ux, uy, uz );
    AnalyticalConst3D<T,T> rho3d(rho);
    sLattice.defineRhoU(superGeometry.getMaterialIndicator({4}), rho3d, u3d);
    sLattice.iniEquilibrium( superGeometry.getMaterialIndicator({4}), rho3d, u3d );
    sLattice.setProcessingContext<Array<momenta::FixedVelocityMomentumGeneric::VELOCITY>>(ProcessingContext::Simulation);
  }
}
...
//    vtmWriter.write( rank );
  ...
  if ( iT%iout == 0 ) {
    ...
    T ymin(converter.getPhysPressure(-amplitude/200));//-amplitude/10000);
    T ymax(converter.getPhysPressure(+amplitude/200));//+amplitude/10000);
    if ( sourcetype == pointsource ) {
      ymin = converter.getPhysPressure(-amplitude/10000);
      ymax = converter.getPhysPressure(+amplitude/10000);
    }
    ...
    if ( iT%(iout*2)==0) {
      ...
      if ( sourcetype == pointsource ) {
        jpeg_ParamP.maxValue = converter.getPhysPressure(+amplitude/100000);
        jpeg_ParamP.minValue = converter.getPhysPressure(-amplitude/100000);
      } else {
...
int main( int argc, char* argv[], char *envp[] )
{
  ...
  T frequency = args.getValueOrFallback( "--frequency", 10);
  ...
  clout << "frequency = " << frequency << " 1/s, while tmax = " << tmax << ", -> " << frequency*tmax << " periods." << std::endl;
  frequency = std::max(frequency, 2./tmax);
  clout << "frequency = " << frequency << " 1/s recalculated for at least 3 periods." << std::endl;
  ...
  while ( iT < imax ) {
    ...
    setBoundaryValues( converter, sLattice, iT, superGeometry, rho0, Ma, boundarytype, source, amplitude, frequency );
