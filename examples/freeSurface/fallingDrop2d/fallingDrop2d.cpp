...
void prepareLattice(MyCase& myCase) {
  ...
  lattice.defineDynamics<SmagorinskyForcedBGKdynamics>( geometry, 1);
...
void getResults(MyCase& myCase, int iT, util::Timer<MyCase::value_t>& timer)
{
  ...
  if ( iT%vtmIter==0 ) {
    ...
    SuperLatticePhysField2D<T, DESCRIPTOR, descriptors::FORCE> force(lattice, converter.getConversionFactorForce());
