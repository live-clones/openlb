...
void simulatePoissonNernstPlanck3D(int N, Gnuplot<T>& gplot)
{
  ...
  CuboidGeometry3D<T> cuboidGeometry( cuboid, converterNernstPlanck.getPhysDeltaX(), singleton::mpi().getSize() );
#else
  CuboidGeometry3D<T> cuboidGeometry( cuboid, converterNernstPlanck.getPhysDeltaX(), 2 );
#endif
